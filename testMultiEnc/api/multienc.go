...
func MultiEncWrite(key []byte, file multipart.File, encFileAddr string) (int64, error) {
	...
	encfile, err := os.Create(encFileAddr)
	if err != nil {
	...
	for {
		...
		countSize += len(encbuf)
	}
	return int64(countSize), nil
}
